...
void LightManager::ApplyLight(std::shared_ptr<ShaderResource> shaderResource, mat4 viewMatrix, mat4 projMatrix) {
...
void LightManager::ApplyDeferredDirLight(std::shared_ptr<GLuint> shaderProgram) {
...
void LightManager::ApplyDeferredPointLight(std::shared_ptr<GLuint> shaderProgram, mat4 viewMatrix, mat4 projMatrix) {
	...
	for (int i = 0; i < this->pointLightV.size(); i++) {
		...
		vec4 currentLightColour = currentLight.colour;
		float maxVal = 0;
		for (int j = 0; j < 3; j++) {
...
void LightManager::AddPointLight(vec3 position, vec4 colour, float intensity) {
	PointLight newPL;
	newPL.pos = position;
	newPL.colour = colour;
...
void LightManager::DrawPointLights(mat4 viewMatrix, mat4 projMatrix) {
	GraphicsNode tempNode;
	ShaderResource shadR;
	tempNode.meshVector.push_back(this->sphere);
	for (int i = 0; i < this->pointLightV.size(); i++) {
...
void LightManager::NewDirLight(vec3 direction, vec4 colour, float intensity) {
	DirLight newDL;
...
void LightManager::SetUp() {
	this->sphere = std::make_shared<MeshResource>();
