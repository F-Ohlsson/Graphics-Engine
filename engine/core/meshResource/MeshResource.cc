	...
	if (objFile.is_open()) {
		...
		while (!objFile.eof()) {
			...
			if ((line[0] == 'v') && (line[1] == ' ')) {
				...
				vec3 vertexData = ProcessLineVec3(line, spaceDelimiter);
			...
			//Begin Texture reading
				...
				vec3 uvData = ProcessLineVec3(line, spaceDelimiter);
			...
			//Begin Normal reading
				...
				vec3 normalData = ProcessLineVec3(line, spaceDelimiter);
	...
	for (int i = 0; i < faceIndices.size(); i++) {
		if (indexTable.find(faceIndices[i].originalString) != indexTable.end()) {
			...
			if (vectorPositions.v[1] != "") {
				vec3 textCoords = textureVector.at(std::stoi(vectorPositions.v[1])-1);
...
vec3 MeshResource::ProcessLineVec3(std::string line, std::string delimiter) {

	vec3 workVector = vec3();
	for (int i = 0; i < 3; i++) {
